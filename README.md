...
- типизированные схемы API на основе `TypedDict`;
...
async def main() -> None:
    async with WaveLinkClient() as client:
        ...
        print("Wave Link:", info.get("version"))

        channels = await client.get_channels()
        for channel in channels:
            print(channel["id"], channel.get("name"))
...
async with WaveLinkClient() as client:
    ...
    for channel in channels:
        print("channel", channel["id"], channel.get("name"))

    for mix in mixes:
        print("mix", mix["id"], mix.get("name"))

    for device in input_devices:
        print("input device", device["id"], device.get("name"))
        for input_ in device.get("inputs", []):
            print("  input", input_["id"], input_.get("name"))

    for device in output_state["outputDevices"]:
        print("output device", device["id"], device.get("name"))
        for output in device.get("outputs", []):
            print("  output", output["id"], output.get("name"))
```
